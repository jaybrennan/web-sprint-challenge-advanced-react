...
To build a stateful class component, you must use a class, constructor function and render function.
For the state, you use a this.state object and place it inside of the constructor. You can then pass state down through the use of props and change state with this.setState().

2. Describe the different phases of the component lifecycle.

There are three phases to the React lifecycle. The first is the Render Phase. In this phase, the constructor is mounted, updated and rendered. This can be run/restarted as state updates. The next step is the Pre-Commit Phase with is where React reads the DOM before the update. During the Commit Phase, React updates the DOM and refs. This includes running side effects and schedule updates.

3. Demonstrate an understanding of class component lifecycle methods.

First, the constructor runs, then render is run for the first time. The component mounts (componentDidMount(){}). Whenever there is a change, the component will re-render with the new changes, and report them (componentDidUpdate(){}). When the component is no longer used inside of the application, it will unmount (componentWillUnmount(){}). 

4. Define stateful logic.

Stateful logic helps us bring our React apps to life. Example usecases are localStorage, useEffect and useState. Essentially whenever you're dealing with stateful logic.

5. Describe how to test a React component with React Testing Library.

With React Testing Library, we have to target whatever we're trying to test. This can be by useing an id, text srting, etc. We can also fire buttons and if we have a function we want to test, we can do that as well. This is all done in a .test.js file.
